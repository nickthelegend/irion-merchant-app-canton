...
import {
  LayoutGrid, CreditCard, Boxes, Wallet, Settings, Loader2, Power, ShieldCheck,
} from "lucide-react"
import { useConnect, useParty, useWalletStatus } from "@/lib/canton-connect-kit"
    ...
    <aside className="w-60 shrink-0 min-h-screen bg-white/[0.02] border-r border-white/10 flex flex-col font-display">
      ...
      <div className="h-20 flex items-center gap-2 px-6 border-b border-white/10">
        ...
        <div className="flex flex-col leading-tight">
          ...
          <span className="text-[9px] text-white/40 uppercase tracking-widest">Merchant · Canton</span>
        </div>
      </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-bold transition-colors ${
                active ? "bg-primary/10 text-primary" : "text-white/55 hover:text-white hover:bg-white/5"
              }`}
            >
        ...
        {isConnected && party ? (
          <div className="flex flex-col gap-2 bg-white/[0.03] border border-white/10 rounded-xl p-3">
            <div className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${isLocked ? "bg-yellow-500" : "bg-green-500"}`} />
              <span className="text-[10px] text-white/60 font-mono uppercase">Carpincho</span>
            </div>
            <code className="text-[10px] text-white/80 break-all" title={party.partyId}>{short(party.partyId)}</code>
            <button
